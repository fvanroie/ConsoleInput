...
inline void ConsoleInput::print_sequence()
{
    ...
    for(int i = 1; i < (int)sizeof(esc_sequence); i++) {
        if(esc_sequence[i] == 0) {
            stream->println();
            if(flags.auto_update) update(__LINE__);
            return;
        }
...
{
    if(stream) return stream->write(c);
    return 0;
}
...
void ConsoleInput::debugShowHistory()
{
    ...
    for(size_t i = 0; i <= num; i++) {
        ...
        if(pos < input_buf_size) stream->println((char*)(input_buf + pos));
    }
}
...
void ConsoleInput::update(size_t line)
{
    ...
    /*if(caret_pos > 0)*/ {
        stream->print("\e[");
        stream->print(caret_pos + _prompt_len); // Move caret to index
        stream->print("C");
    }
}
...
int16_t ConsoleInput::readKey()
{
    ...
    if(key == 0x1b) { /* start escape sequence */
        ...
        switch(key) {
            ...
            case '[': { // CSI mode
                ...
                switch(appended_key) {
                    ...
                    case 'C':
                        if(flags.auto_move) setCaret(caret_pos + 1);
                        end_sequence();
                        return KEY_RIGHT;
                    ...
                    default:
                        // should not happen
                        if(stream != NULL) stream->println(F("WARNING !!!"));
            ...
            case 'O': { // CSO mode
                ...
                switch(key) {
                    ...
                    case 'S':
                        ...
                        return KEY_FN + 4;
                    default:
                        if(stream != NULL) stream->println(F("UNKNOWN"));
                        return KEY_UNKNOWN;
                }
                break;
            } // CSO mode
    ...
    end_sequence();
    if(key == 0x7f) key = 7; // DEL = BACKSPACE

    if(key >= 0x20 && key <= 0xff) { // printable characters
        if(flags.auto_edit) insertCharacter(key);
        return key;
    } else if(key >= 0x00 && key < 0x20) {

        switch(key) {           // Ctrl + CHAR
            ...
            case KEY_CTRL('H'): // Backspace
                if(flags.auto_edit) do_backspace();
                return KEY_BACKSPACE;

            case KEY_LF ... KEY_CR: { // LF, VT, FF, CR
                // handle CR/LF
                if(key == KEY_CR && stream != NULL && stream->peek() == KEY_LF) stream->read();
